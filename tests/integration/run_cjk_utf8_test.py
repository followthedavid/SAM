...
def run():
    ...
    fix_path = os.path.join(os.path.dirname(__file__), "fixtures", "cjk_utf8.raw")
    ...
    # Check for CJK characters in output
    output_str = json.dumps(arr)
    has_cjk = any(ord(c) > 0x4E00 for c in output_str)
    assert has_cjk or len(arr) >= 1, "Expected CJK output or at least one block"
    
    print(f"✅ CJK UTF-8 test OK — parsed {len(arr)} blocks")
