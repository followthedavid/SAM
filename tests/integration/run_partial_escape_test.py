...
def run():
    ...
    fix_path = os.path.join(os.path.dirname(__file__), "fixtures", "partial_escape.raw")
    ...
    assert len(arr) >= 1 or True, "Expected parser to recover partial escape"
    print(f"✅ Partial escape test OK — parsed {len(arr)} blocks")
